...
import { parseStudentCsv, type StudentCsvRow } from "@/lib/admin/csv-parser";
import { importStudentsFromCsvAction } from "@/app/(admin)/admin/students/actions";

interface AdminStudentCsvImportProps {
  academicYear: number;
}

export function AdminStudentCsvImport({
  academicYear,
}: AdminStudentCsvImportProps) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      const importResult = await importStudentsFromCsvAction(formData);
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>生徒CSVインポート（{academicYear}年度）</DialogTitle>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          <div>
            <p className="text-muted-foreground mb-2 text-sm">
              CSV形式: 学年,組,出席番号,氏名,氏名カナ,Googleメールアドレス
            </p>
            <p className="text-muted-foreground mb-2 text-xs">
              ※クラスが存在しない場合は自動作成されます。既存メールアドレスの場合は情報を更新します。
            </p>
            <input
              type="file"
              accept=".csv"
                  ...
                  <tbody>
                    {preview.slice(0, 20).map((row, i) => (
                      <tr key={i} className="border-b last:border-0">
                        ...
                        <td className="px-2 py-1">{row.studentNumber}</td>
